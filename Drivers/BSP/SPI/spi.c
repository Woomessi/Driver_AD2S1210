...
SPI_HandleTypeDef g_spi4_handler;                                      /* SPI1句柄 */
...
void spi4_init(void)
{
    SPI4_SPI_CLK_ENABLE();                                             /* SPI1时钟使能 */

    g_spi4_handler.Instance = SPI4_SPI;                                /* SPI1 */
    g_spi4_handler.Init.Mode = SPI_MODE_MASTER;                        /* 设置SPI工作模式，设置为主模式 */
    g_spi4_handler.Init.Direction = SPI_DIRECTION_2LINES;              /* 设置SPI单向或者双向的数据模式:SPI设置为双线模式 */
    g_spi4_handler.Init.DataSize = SPI_DATASIZE_8BIT;                  /* 设置SPI的数据大小:SPI发送接收8位帧结构 */
    g_spi4_handler.Init.CLKPolarity = SPI_POLARITY_HIGH;               /* 串行同步时钟的空闲状态为高电平 */
    g_spi4_handler.Init.CLKPhase = SPI_PHASE_2EDGE;                    /* 串行同步时钟的第二个跳变沿（上升或下降）数据被采样 */
    g_spi4_handler.Init.NSS = SPI_NSS_SOFT;                            /* NSS信号由硬件（NSS管脚）还是软件（使用SSI位）管理:内部NSS信号有SSI位控制 */
    g_spi4_handler.Init.BaudRatePrescaler = SPI_BAUDRATEPRESCALER_4;   /* 定义波特率预分频的值:波特率预分频值为4 */
    g_spi4_handler.Init.FirstBit = SPI_FIRSTBIT_MSB;                   /* 指定数据传输从MSB位还是LSB位开始:数据传输从MSB位开始 */
    g_spi4_handler.Init.TIMode = SPI_TIMODE_DISABLE;                   /* 关闭TI模式 */
    g_spi4_handler.Init.CRCCalculation = SPI_CRCCALCULATION_DISABLE;   /* 关闭硬件CRC校验 */
    g_spi4_handler.Init.CRCPolynomial = 10;                            /* CRC值计算的多项式 */
    HAL_SPI_Init(&g_spi4_handler);                                     /* 初始化 */

    __HAL_SPI_ENABLE(&g_spi4_handler);                                 /* 使能SPI1 */

    spi4_read_write_byte(0XFF);                                        /* 启动传输, 实际上就是产生8个时钟脉冲, 达到清空DR的作用, 非必需 */
}
...
void HAL_SPI_MspInit(SPI_HandleTypeDef *hspi)
{
    ...
    if (hspi->Instance == SPI4_SPI)
    {
        SPI4_SCK_GPIO_CLK_ENABLE();  /* SPI1_SCK脚时钟使能 */
        SPI4_MOSI_GPIO_CLK_ENABLE(); /* SPI1_MOSI脚时钟使能 */
			  SPI4_MISO_GPIO_CLK_ENABLE(); /* SPI1_MOSI脚时钟使能 */
...
{
    assert_param(IS_SPI_BAUDRATE_PRESCALER(speed)); /* 判断有效性 */
    __HAL_SPI_DISABLE(&g_spi4_handler);             /* 关闭SPI */
    g_spi4_handler.Instance->CR1 &= 0XFFC7;         /* 位3-5清零，用来设置波特率 */
    g_spi4_handler.Instance->CR1 |= speed << 3;     /* 设置SPI速度 */
    __HAL_SPI_ENABLE(&g_spi4_handler);              /* 使能SPI */
}
/**
 * @brief       SPI1读写一个字节数据
...
{
    uint8_t rxdata;
    HAL_SPI_TransmitReceive(&g_spi4_handler, &txdata, &rxdata, 1, 1000);
    return rxdata; /* 返回收到的数据 */
}
...
void spi4_write_data(uint8_t *tx_data, uint16_t size)
{
    HAL_SPI_Transmit(&g_spi4_handler, tx_data, size, HAL_MAX_DELAY);
}
