...
void norflash_write(uint8_t *pbuf, uint32_t addr, uint16_t datalen)
{
    ...
    while (1)
    {
        sw_spiflash_mode();                     /* 切换为FLASH，在USB读写大数据的时候不切换会导致屏幕黑屏 */
 ...
 * @note        因为lcd和spi_flash共用一个SPI1，当从lcd切换回spiflash时需要对引脚功能进行配置，切换回SPI_MISO功能
 * @param       无
 * @retval      无
 */
...
/**
 * @brief       切换为lcd模式
 * @note        因为lcd和spi_flash共用，当从spiflash切换回lcd时需要对引脚功能进行配置，切换回lcd模式
 * @param       无
 * @retval      无
 */
