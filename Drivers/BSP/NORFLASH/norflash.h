...
#define NORFLASH_CS_GPIO_PIN            GPIO_PIN_8
#define NORFLASH_CS_GPIO_CLK_ENABLE()   do{ __HAL_RCC_GPIOB_CLK_ENABLE(); }while(0)   /* PB口时钟使能 */

/* PB4被LCD与SPI_FLASH共用，所以使用时需进行分时复用，在读写SPIFLASH时使用到 */
...
void sw_lcd_mode(void);                                                 /* 切换为LCD模式 */
void sw_spiflash_mode(void);                                            /* 切换为SPI_FLASH模式 */
#endif
